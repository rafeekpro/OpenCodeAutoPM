/**
 * PRD Parse Command
 * Hybrid: Basic epic generation (deterministic) + AI decomposition (OpenCode)
 */
...
## Source PRD
- **Document**: .opencode/prds/$PRD_NAME.md
- **Created**: $PRD_DATE
...
exports.builder = (yargs) => {
  return yargs
    ...
    .example('/pm:prd-parse user-auth', 'AI-powered decomposition in OpenCode');
};
...
exports.handler = async (argv) => {
  ...
  try {
    ...
    // Check if PRD exists
    const prdPath = path.join(process.cwd(), '.opencode', 'prds', 'drafts', `${argv.feature_name}.md`);
    const oldPrdPath = path.join(process.cwd(), '.opencode', 'prds', `${argv.feature_name}.md`);
    ...
    const epicDir = path.join(process.cwd(), '.opencode', 'epics');
    ...
    if (argv.basic) {
      ...
      spinner.succeed();
      printSuccess(`✅ Epic created: .opencode/epics/${argv.feature_name}.md`);
      console.log();
    ...
    console.log('╚════════════════════════════════════════════════╝');
    console.log();
    printWarning('This command requires OpenCode for intelligent PRD parsing');
    console.log();

    printInfo('📍 To parse PRD with AI assistance:');
    console.log(`   In OpenCode, run: \`/pm:prd-parse ${argv.feature_name}\``);
    console.log();
    ...
    console.log('   .opencode/commands/pm/prd-parse.md');
