...
class InstallHooks {
  ...
  async run(args = []) {
    ...
    const hooks = [
      {
        name: 'pre-push',
        source: '.opencode/hooks/pre-push-docker-tests.sh'
      },
      {
        name: 'pre-commit',
        source: '.opencode/hooks/docker-first-enforcement.sh'
      }
    ];
    ...
    if (availableHooks.length === 0) {
      this.print('No hook files found in .opencode/hooks/', 'yellow');
