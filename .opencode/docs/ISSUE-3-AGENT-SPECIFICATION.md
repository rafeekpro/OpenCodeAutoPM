...
| JavaScript/Node.js | `.opencode/agents/languages/nodejs-backend-engineer.md` |
...
| GitHub Actions | `.opencode/agents/devops/github-operations-specialist.md` |
...
**Process**:
1. **Read PRD** from `.opencode/prds/$ARGUMENTS.md`
...
**After** (line 297-322):
```markdown
**IMPORTANT**: Do NOT use generic "general-purpose" agents. Task creation should be done directly by OpenCode, not delegated to sub-agents, as it requires careful analysis of PRD, agent selection, and proper frontmatter generation.
...
assigned_agent: .opencode/agents/databases/redis-expert.md
agent_context:
  pattern: cache-aside
  ttl: 3600
```
...
```yaml
required_agents:
  - path: .opencode/agents/frontend/react-frontend-engineer.md
    role: Dashboard UI development
    tasks: [001, 002, 003]
  - path: .opencode/agents/languages/nodejs-backend-engineer.md
    role: API implementation
    tasks: [004, 005]
  - path: .opencode/agents/databases/mongodb-expert.md
    role: Database schema and queries
    tasks: [006]
  - path: .opencode/agents/testing/frontend-testing-engineer.md
    role: Component and E2E tests
    tasks: [007, 008]
  - path: .opencode/agents/cloud/aws-cloud-architect.md
...
# Step 1: Read epic from .opencode/epics/user-authentication/epic.md
# Step 2: Read PRD from .opencode/prds/user-authentication.md
...
# Read task file: .opencode/epics/user-authentication/46.md
# Extract: assigned_agent = .opencode/agents/languages/python-backend-engineer.md
...
- [ ] Agent paths are valid (.opencode/agents/{category}/{name}.md)
...
2. ✅ `.opencode/docs/ISSUE-3-AGENT-SPECIFICATION.md` - This documentation (DONE)
