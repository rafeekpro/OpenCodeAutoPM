...
OpenCode users experience **significant context memory consumption** from AutoPM's comprehensive rules system:
...
💡 Next steps:
   ...
   • Restore files if needed: mv .opencode/rules-archive/<file> .opencode/rules/
...
- All archived files stored in `.opencode/rules-archive/`
- Restore individual files: `mv .opencode/rules-archive/file.md .opencode/rules/`
...
mv .opencode/rules-archive/ci-cd-kubernetes-strategy.md .opencode/rules/
mv .opencode/rules-archive/cloud-security-compliance.md .opencode/rules/
mv .opencode/rules-archive/infrastructure-pipeline.md .opencode/rules/
...
2. All 34 rules copied to .opencode/rules/
...
Both issues address **OpenCode usability blockers**:

| Aspect | Issue #2 (Interactive Prompts) | Issue #6 (Context Optimization) |
|--------|-------------------------------|--------------------------------|
| **Problem** | Commands hang in OpenCode | High memory usage slows responses |
...
Add interactive wizard to reduce OpenCode context memory usage
...
- Result: Faster OpenCode responses, more context available
...
- Created: `.opencode/docs/ISSUE-6-CONTEXT-OPTIMIZATION-WIZARD.md` (this file)
