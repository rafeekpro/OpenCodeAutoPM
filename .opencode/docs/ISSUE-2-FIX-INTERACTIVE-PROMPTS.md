# Issue #2 Fix: Interactive Prompts Blocking OpenCode
...
The `/pm:prd-new` command used interactive readline prompts by default, which **completely blocked OpenCode users** because:
1. OpenCode terminal is non-interactive
2. Users couldn't type responses to prompts
3. Command would hang indefinitely
4. No workaround available

**User Report**:
> "There is an issue with PRDs. It always expects some interactive response when we are using it in OpenCode. Therefore we cannot answer in the terminal."
...
**Before** (Broken):
- Default: Interactive prompts → Blocks OpenCode ❌
- `--content`: Non-interactive → Works ✅

**After** (Fixed):
- Default: LLM-assisted generation → Works in OpenCode ✅
- `--interactive`: Interactive prompts → Terminal only ✅
- `--content`: Non-interactive → Works ✅
...
4. Added warning about `--interactive` not working in OpenCode
...
3. Prevents script from hanging in OpenCode

**Code Added** (lines 615-627):
```javascript
// Check if running in non-interactive environment (e.g., OpenCode)
if (!process.stdin.isTTY) {
  ...
  console.error('   3. Run in a regular terminal (not OpenCode):');
...
**Behavior**:
1. ✅ Command file instructs Claude to generate PRD content
2. ✅ Claude analyzes project context and technology stack
3. ✅ Claude writes comprehensive PRD using template
4. ✅ No terminal interaction required
5. ✅ Works perfectly in OpenCode
...
5. ⚠️ Only works in interactive terminals (not OpenCode)
...
3. Writes to `.opencode/prds/feature.md`
4. ✅ Works everywhere (OpenCode, terminal, CI/CD)
...
- OpenCode users can now create PRDs
...
### Test Case 1: OpenCode (Default Mode)
```bash
# In OpenCode
/pm:prd-new test-feature

# Expected: ✅ PRD generated by LLM
...
# Actual: ✅ Traditional wizard launches
```

### Test Case 3: Non-Interactive Script Call (Error Handling)
```bash
# In OpenCode (trying to call script directly)
node .opencode/scripts/pm/prd-new.js test-feature

# Expected: ❌ Clear error message with solutions
# Actual: ✅ Shows helpful error and alternatives
```
...
# Expected: ✅ Reads file and creates PRD
...
1. ✅ **DONE**: Fix interactive prompts
2. ⏭️ **NEXT**: Implement context optimization wizard (Issue #6)
3. ⏭️ **THEN**: Add pre-PRD codebase analysis (Issue #1)
...
Changes:
- Add LLM-assisted generation as default mode (works in OpenCode)
...
Fixes: #2 - Interactive prompts blocking OpenCode users

Impact:
- ✅ OpenCode users can now create PRDs
- ✅ Better quality PRDs with project context analysis
- ✅ Clear error messages for misconfiguration
- ✅ No breaking changes (--interactive preserves old behavior)
