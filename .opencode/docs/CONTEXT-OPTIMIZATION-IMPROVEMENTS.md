...
class ContextDashboard {
  ...
  scanProject() {
    ...
    // Rules
    this.scanDirectory('.opencode/rules', 'rules', '*.md');

    // Agents
    this.scanDirectory('.opencode/agents', 'agents', '*.md');

    // Documentation
    this.scanFile('CLAUDE.md', 'docs');
    this.scanFile('.opencode/DEVELOPMENT-STANDARDS.md', 'docs');
  ...
  display() {
    ...
    console.log('┌─────────────────────────────────────────────────────────────┐');
    console.log('│ 📊 OpenCode Context Usage Dashboard                      │');
    console.log('├─────────────────────────────────────────────────────────────┤');
    console.log('│                                                              │');
  ...
  isAgentUsed(agentPath) {
    ...
    try {
      const result = execSync(
        `grep -r "${path.basename(agentPath)}" .opencode/epics/ 2>/dev/null || true`,
        { encoding: 'utf8' }
      );
  ...
  exportJSON() {
    ...
    const outputPath = '.opencode/context-analysis.json';
...
```javascript
// .opencode/scripts/smart-context-loader.js
...
class SmartContextLoader {
  ...
  scanPRDForTechnologies() {
    ...
    const technologies = {
      ...
      'postgres': ['.opencode/agents/databases/postgresql-expert.md'],
      // ... etc
    };
...
budget:
  ...
  allocations:
    ...
    core:
      ...
      items:
        - CLAUDE.md
        - .opencode/rules/golden-rules.md
        - .opencode/rules/tdd.enforcement.md
...
# → Opens: .opencode/context-heatmap.html in browser
```
