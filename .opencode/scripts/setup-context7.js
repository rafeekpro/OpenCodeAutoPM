...
class SetupContext7 {
  ...
  // Setup .env file
  setupEnvFile() {
    const envFile = '.opencode/.env';
    const envExample = '.opencode/.env.example';
    const claudeDir = '.opencode';
    ...
    if (!fs.existsSync(claudeDir)) {
      try {
        ...
        this.print(`❌ Failed to create .opencode directory: ${error.message}`, 'red');
        return false;
      }
    }
    ...
    if (!fs.existsSync(envFile)) {
      ...
      if (fs.existsSync(envExample)) {
        try {
          ...
          this.print('✅ Created .opencode/.env file', 'green');
        } catch (error) {
...
`;
        try {
          ...
          this.print('✅ Created .opencode/.env file with defaults', 'green');
        } catch (error) {
          this.print(`❌ Failed to create .env file: ${error.message}`, 'red');
          return false;
        }
      ...
      console.log('');
      this.print('❗ IMPORTANT: Edit .opencode/.env and add your actual Context7 credentials:', 'yellow');
  ...
  testConnection() {
    ...
    if (env.CONTEXT7_API_KEY && env.CONTEXT7_API_KEY !== 'your-context7-api-key-here') {
      ...
      console.log('   Please edit .opencode/.env with your actual API key and workspace');
    }
  }
