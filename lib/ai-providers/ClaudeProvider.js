/**
 * ClaudeProvider - Anthropic OpenCode API Integration
 *
...
/**
 * ClaudeProvider class for Anthropic OpenCode API integration
 *
...
class ClaudeProvider extends AbstractAIProvider {
   ...
   *
   * @returns {string} Default OpenCode model
   */
  getDefaultModel() {
  ...
  /**
   * Check if OpenCode supports streaming
   *
   * @returns {boolean} True (OpenCode supports streaming)
   */
  supportsStreaming() {
    return true;
  }
  ...
  /**
   * Check if OpenCode supports function calling
   *
   * @returns {boolean} True (OpenCode supports tools/function calling)
   */
  ...
  /**
   * Check if OpenCode supports chat format
   *
   * @returns {boolean} True (OpenCode has native chat format)
   */
  supportsChat() {
    return true;
  }
  ...
  /**
   * Check if OpenCode supports vision/image inputs
   *
      ...
      switch (error.status) {
        ...
        case 429:
          return new AIProviderError(
            ...
            'OpenCode API rate limit exceeded. Please retry after a delay.',
            true,
            429
          );
        ...
        case 504:
          return new AIProviderError(
            ...
            `OpenCode API service temporarily unavailable (${error.status})`,
            true,
            error.status
          );
   ...
   * Converts system role to user role (OpenCode requirement).
   *
   * Automatically applies rate limiting if configured in constructor.
   *
  ...
  async chat(messages, options = {}) {
    return this._withRateLimit(async () => {
      ...
      try {
        // Convert system role to user (OpenCode doesn't support system role in messages array)
