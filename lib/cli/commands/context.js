 ...
 *
 * Provides context management commands for OpenCodeAutoPM.
...
async function contextCreate(argv) {
  ...
  try {
    ...
    console.log(`  ${chalk.cyan('1.')} View context:     ${chalk.yellow(`open-autopm context show ${argv.type}`)}`);
    console.log(`  ${chalk.cyan('2.')} Update context:   ${chalk.yellow(`open-autopm context update ${argv.type}`)}`);
    console.log(`  ${chalk.cyan('3.')} List all:         ${chalk.yellow('open-autopm context show --list')}\n`);
...
async function contextPrime(argv) {
  ...
  try {
    ...
    console.log(`  ${chalk.cyan('1.')} Use snapshot in OpenCode conversations`);
    console.log(`  ${chalk.cyan('2.')} Update specific contexts: ${chalk.yellow('open-autopm context update <type>')}`);
    console.log(`  ${chalk.cyan('3.')} View all contexts:        ${chalk.yellow('open-autopm context show --list')}\n`);
...
async function contextUpdate(argv) {
  ...
  try {
    ...
    if (!content) {
      ...
      console.log(`  ${chalk.yellow(`open-autopm context update ${argv.type} --content "New content"`)}`);
      console.log(`  ${chalk.yellow(`open-autopm context update ${argv.type} --file updates.md`)}\n`);
      return;
    }
    ...
    console.log(chalk.bold('💡 Next Steps:\n'));
    console.log(`  ${chalk.cyan('1.')} View updated context: ${chalk.yellow(`open-autopm context show ${argv.type}`)}`);
    console.log(`  ${chalk.cyan('2.')} Generate snapshot:    ${chalk.yellow('open-autopm context prime')}\n`);
    ...
    if (error.message.includes('not found')) {
      ...
      console.error(chalk.gray(`  Use: ${chalk.yellow('open-autopm context show --list')}\n`));
    } else {
      console.error(chalk.red(`\nError: ${error.message}\n`));
    }
    process.exit(1);
...
async function contextShow(argv) {
  ...
  if (argv.list) {
    ...
    try {
      ...
      if (contexts.length === 0) {
        ...
        console.log(`   ${chalk.yellow('open-autopm context create project-brief --name "My Project"')}\n`);
        return;
      }
  ...
  if (!argv.type) {
    ...
    console.log(chalk.bold('Usage:'));
    console.log(`  ${chalk.yellow('open-autopm context show <type>')}`);
    console.log(`  ${chalk.yellow('open-autopm context show --list')}\n`);
    console.log(chalk.bold('Examples:'));
    console.log(`  ${chalk.gray('open-autopm context show project-brief')}`);
    console.log(`  ${chalk.gray('open-autopm context show --list')}`);
    console.log(`  ${chalk.gray('open-autopm context show --list --stats')}\n`);
    return;
  }
  ...
  try {
    ...
    console.log(`  ${chalk.cyan('1.')} Update context:  ${chalk.yellow(`open-autopm context update ${argv.type}`)}`);
    console.log(`  ${chalk.cyan('2.')} List all:        ${chalk.yellow('open-autopm context show --list')}\n`);
    ...
    .command(
      ...
      (yargs) => {
        return yargs
          ...
          .example('open-autopm context create project-brief --name "My Project"', 'Create project brief')
          .example('open-autopm context create progress --name "Sprint 1"', 'Create progress tracker');
      },
      contextCreate
    )
    .command(
      ...
      (yargs) => {
        return yargs
          ...
          .example('open-autopm context prime', 'Generate project snapshot')
          .example('open-autopm context prime --output snapshot.md', 'Save snapshot to file')
          .example('open-autopm context prime --no-include-git', 'Skip git information');
      },
      contextPrime
    )
    .command(
      ...
      (yargs) => {
        return yargs
          ...
          .example('open-autopm context update project-brief --content "## New Section"', 'Append content')
          .example('open-autopm context update progress --file updates.md', 'Update from file')
          .example('open-autopm context update tech-context --mode replace --content "..."', 'Replace content');
      },
      contextUpdate
    )
    .command(
      ...
      (yargs) => {
        return yargs
          ...
          .example('open-autopm context show project-brief', 'Show specific context')
          .example('open-autopm context show --list', 'List all contexts')
          .example('open-autopm context show --list --stats', 'List with statistics');
      },
      contextShow
    )
...
module.exports = {
  ...
  handler: (argv) => {
    if (!argv._.includes('context') || argv._.length === 1) {
      ...
      console.log('Usage: open-autopm context <command>\n');
      ...
      console.log('\nUse: open-autopm context <command> --help for more info\n');
    }
  },
  handlers: {
