...
class TemplateEngine {
  constructor(builtInDir, userDir) {
    ...
    // In production, use .opencode/templates (framework templates are copied during installation)
    this.builtInDir = builtInDir || path.join(__dirname, '..', '.opencode', 'templates');
    this.userDir = userDir || path.join('.opencode', 'templates');
  }
  ...
  render(template, variables) {
    ...
    if (!variables.id && !autoVars.id) {
      ...
      const directory = `.opencode/${type}s`;
