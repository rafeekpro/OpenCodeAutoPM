...
class ConflictHistory {
   ...
   * @param {string} options.storagePath - Path to storage file (default: '.opencode/.conflict-history.json')
   */
  constructor(options = {}) {
    // Validate and sanitize storagePath to prevent path traversal
    let storagePath = options.storagePath || '.opencode/.conflict-history.json';
