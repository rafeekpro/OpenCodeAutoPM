 ...
 *
 * Pure service layer for epic operations following OpenCodeAutoPM patterns.
...
class EpicService {
   ...
   * @param {string} [options.epicsDir] - Path to epics directory (default: .opencode/epics)
  ...
  constructor(options = {}) {
    ...
    this.options = {
      epicsDir: options.epicsDir || '.opencode/epics',
   ...
   * //   progress: '0%',
   * //   prd: '.opencode/prds/user-auth.md',
...
${prdContent}`;
  ...
  async _loadEpicSyncMap() {
    const fs = require('fs-extra');
    const path = require('path');
    const syncMapPath = path.join(process.cwd(), '.opencode/epic-sync-map.json');
...
`;
  ...
  async _loadAzureEpicSyncMap() {
    const fs = require('fs-extra');
    const path = require('path');
    const syncMapPath = path.join(process.cwd(), '.opencode/epic-azure-sync-map.json');
